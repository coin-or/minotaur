...
bool PCBProcessor::presolveNode_(NodePtr node, SolutionPoolPtr s_pool)
{
  ...
  for(it = 0; it < max_iter && true == cont; ++it) {
    ++it;
    for(HandlerIterator h = handlers_.begin();
        h != handlers_.end() && false == is_inf; ++h) {
...
void PCBProcessor::removeCuts_(ConstSolutionPtr sol)
{
  HandlerIterator h;
  for(h = handlers_.begin(); h != handlers_.end(); ++h) {
    (*h)->removeCuts(relaxation_, sol);
  }
}
  ...
  case(ProvenOptimal):
    ...
    if(node->getParent() && node->getParent()->getLb() > solval + oATol_ &&
      ...
      logger_->msgStream(LogError) << me_ << "node lb lower than parent's lb. "
                                   ...
                                   << "engine has an issue." << std::endl;
    }
    best_cutoff = s_pool->getBestSolutionValue();
    if(solval >= best_cutoff - oATol_ ||
       solval >= best_cutoff - fabs(best_cutoff) * oRTol_ ||
       solval >= cutOff_) {
      node->setStatus(NodeHitUb);
      should_prune = true;
      ++stats_.ub;
...
{
  out << me_ << "nodes processed                       = " << stats_.proc
      << std::endl
      << me_ << "nodes branched                        = " << stats_.bra
      << std::endl
      << me_ << "nodes infeasible                      = " << stats_.inf
      << std::endl
      << me_ << "nodes optimal                         = " << stats_.opt
      << std::endl
      << me_ << "nodes hit ub                          = " << stats_.ub
      << std::endl
      << me_ << "nodes with problems                   = " << stats_.prob
      << std::endl
      << me_ << "nodes for which fixNodeErr was called = " << stats_.tol_err
      << std::endl;
}
