...
CoverCutGenerator::CoverCutGenerator(RelaxationPtr rel, ConstSolutionPtr sol,
                                     EnvPtr env)
// : stats_(0)
{
  //ProblemPtr p;
...
bool CoverCutGenerator::initialize(RelaxationPtr p, ConstSolutionPtr s,
                                   EnvPtr env)
{
...
void CoverCutGenerator::generateAllCuts()
{
  if (knapsackListPtr_->getNumKnaps() >= 1) {
    ...
    cerr << "CoverCutGenerator::generateAllCuts. No constraint used for cut "
            "generation.\n";
  }
}
                                    ...
                                    CoverSetPtr zerovars)
{
  ...
  for (it = begin; it != end; ++it) {
    ...
    // Serdar check this, this should take value of variable as second
    // element.!!!!
...
void CoverCutGenerator::sortReducedCosts(CoverSetPtr &vars)
{
  if (vars->empty() == false) {
    ...
    for (it = begin; it != end; ++it) {
      ...
      // Very important!!! negative of reduced cost is used to not to
      // construct another compare for sort. Better to construct a separate
      // compares.
...
CoverSetPtr CoverCutGenerator::coverSetGeneratorDefault(
    ConstConstraintPtr cons)
{
  ...
  if (DEBUG_LEVEL >= 9) {
    printIneq(def, (double)(def->size()) - 1, Set,
              "Default cover generated.");
  }
                                             ...
                                             CoverSetPtr cover)
{
  ...
  // Sort variables in nonincreasing order of their values in the given
  // solution.
  ...
  if (sum <= b) {
    ...
    if (DEBUG_LEVEL >= 9) {
      printIneq(cover, (double)(cover->size()) - 1, Cover,
                "GNS initial cover generated.");
    }
  ...
  do {
    ...
    if (min < difference) {
      ...
      break;
           ...
           b);  // make sure that we do not erase variables such that sum
                // =< b
}
...
void CoverCutGenerator::coverPartitionGNS(const ConstConstraintPtr cons,
                                          ...
                                          CoverSetPtr cone, CoverSetPtr ctwo,
                                          CoverSetPtr fset, CoverSetPtr rset)
{
  ...
  size_t numremaining = cover->size();
  ...
  if (DEBUG_LEVEL >= 9) {
    printIneq(cover, (double)cover->size() - 1, Set,
              "Minimal cover for GNS cover partition.\n C");
    printIneq(cbar, (double)cbar->size() - 1, Set, "N-C");
    printIneq(cone, (double)cone->size() - 1, Set, "C1");
    printIneq(ctwo, (double)ctwo->size() - 1, Set, "C2");
    printIneq(fset, (double)fset->size() - 1, Set, "F");
    printIneq(rset, (double)rset->size() - 1, Set, "R");
  }
}

/* This function generates set N\C, the variables outside of cover set.
 */
void CoverCutGenerator::cBar(const ConstCoverSetPtr cover, CoverSetPtr cbar,
...
double CoverCutGenerator::lift(const ConstCoverSetPtr obj,
                               ...
                               double &rhs, double &initialb, bool uplift)
{
  ...
  if (uplift == true) {
    ...
    if (DEBUG_LEVEL >= 9) {
      printLiftProb(obj, constraint, variable, rhs, initialb, uplift, b,
                    gamma, alpha);
    }
                               ...
                               const ConstConstraintPtr cons)
{
  ...
  if ((cbar->empty() == false)) {
    ...
    double rhs = (double)coverineq->size() - 1;
                                ...
                                const ConstConstraintPtr cons)
{
  ...
  for (it = begin; it != end; ++it) {
    ...
    double rhs = (double)lifted->size() - 1;
...
CutPtr CoverCutGenerator::generateCut(const ConstCoverSetPtr constraint,
                                      const double ub)
{
...
bool CoverCutGenerator::addCut(CoverSetPtr cov, double rhs, UInt cuttype)
{
  ...
  if (DEBUG_LEVEL >= 9) {
    ...
    switch (cuttype) {
    case Extended:
      printIneq(cov, rhs, Cons, "Cut " + numc + " extended");
      break;
    case Simple:
      printIneq(cov, rhs, Cons, "Cut " + numc + " simple");
      break;
    case Gns:
      printIneq(cov, rhs, Cons, "Cut " + numc + " gns");
      break;
    case Singlectwo:
      printIneq(cov, rhs, Cons, "Cut " + numc + " singlwctwo");
      break;
    case Basic:
      printIneq(cov, rhs, Cons, "Cut " + numc + " basic");
      break;
    default:
      // New
      output_->open(outfile_.c_str(), std::ios_base::app);
      (*output_) << "CoverCutGenerator.cpp::addCut. Undefined cut type."
                 << endl;
      output_->close();
    }
  }
...
// This may cause a problem at the higher level when we check duplicacy in Cut
// manager.
bool CoverCutGenerator::checkExists(CoverSetPtr cov, double rhs)
{
  ...
  if (found == endmap) {
    cutmap.insert(
        std::pair<std::vector<double>, UInt>(coeffs, stats_->totalcuts));
    return false;
  } else {
    return true;
  }
...
void CoverCutGenerator::generateCuts(ConstConstraintPtr cons)
{
  ...
  // I guess this does not generate different cuts from initial cover
  // inequalities.
...
bool CoverCutGenerator::liftingGNS(const ConstCoverSetPtr cone,
                                   ...
                                   const ConstConstraintPtr cons, double &rhs)

{
  // Initialize the needed data elements.
  // rhs of cover inequality.
  rhs = (double)cone->size() - 1;
 ...
 */
void CoverCutGenerator::liftSet(CoverSetPtr obj, CoverSetPtr consknap,
                                const ConstCoverSetPtr varset,
                                CoverSetPtr constraint, double &rhs,
                                double &initialb, bool liftup)
{
 ...
 */
void CoverCutGenerator::liftSetF(CoverSetPtr obj, CoverSetPtr consknap,
                                 const ConstCoverSetPtr setf,
                                 CoverSetPtr coverineq, double &rhs,
                                 double &initialb, const bool liftup)
{
  ...
  if (setf->empty() == false) {
    ...
    while (fin->empty() == false) {
      ...
      for (it = begin; it != end; ++it) {
        ...
        // changed!!!.
        double alpha = lift(objlocal, constknaplocal, varlocal, rhslocal,
                            initblocal, liftup);
...
void CoverCutGenerator::extendedCover(CoverSetPtr cover,
                                      ConstConstraintPtr cons)
{
  const LinearFunctionPtr lf = cons->getLinearFunction();
  ...
  // Here, we add the variables according to nonincreasing order of
  // coefficients. We can use CBAR, by this way it may work faster.
...
// Assumption: Items are sorted in the order of c_1/a_1 >= c_2/a_2 >= ...
// c_n/a_n
UInt CoverCutGenerator::binaryKnapsackSolver(size_t n, double b,
...
void CoverCutGenerator::printIneq(const ConstCoverSetPtr cover, double rhs,
                                  PrintType type, string message)
{
  ...
  if (cover->empty()) {
    switch (type) {
    case Cover:
      (*output_) << "Empty cover." << endl;
      break;
    case Cons:
      (*output_) << "Empty constraint." << endl;
      break;
    case Obj:
      (*output_) << "Empty objective." << endl;
      break;
    case Set:
      (*output_) << "Empty set." << endl;
      break;
    default:
      (*output_) << "CoverCutGenerator.cpp::printIneq. Invalid print type."
                 << endl;
    }
  } else {
    ...
    for (it = begin; it != end; ++it) {
      ...
      case Set:
        ...
        break;
      default:
        cerr << "CoverCutGenerator.cpp::printIneq. Invalid print type."
             << endl;
      }  // end of switch.
      first = false;
    }  // end of for.
...
void CoverCutGenerator::printLiftProb(const ConstCoverSetPtr obj,
                                      ...
                                      const CoverSetConstIterator variable,
                                      double rhs, double, bool uplift,
                                      double b, double gamma, double alpha)
{
  output_->close();
  output_->open(outfile_.c_str(), std::ios_base::app);
  // Add two lines empty.
