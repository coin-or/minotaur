 ...
 * \author Prashant Palkar, Meenarli Sharma and Ashutosh Mahajan, IIT Bombay
 */


#ifndef MINOTAURPARRELIABILITYBRANCHER_H
#define MINOTAURPARRELIABILITYBRANCHER_H
...
class ParReliabilityBrancher : public Brancher {
...
public:
  ...
  UIntVector getLastStrBranched() {return lastStrBranched_;}
  ...
  /// Return the vector of last strong branching information of candidates.
  void setLastStrBranched(UIntVector lastStrBranched) {lastStrBranched_ = lastStrBranched ;};
   ...
   * the parental chain (direct ancestors only).
   */
  void setPCUp(DoubleVector psuedoUp) { pseudoUp_ = psuedoUp; };

  /**
   * Return the vector of pseudocosts of down-branchings upto this node in
   * the parental chain (direct ancestors only).
   */
  void setPCDown(DoubleVector psuedoDown) { pseudoDown_ = psuedoDown; };
