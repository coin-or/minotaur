...
NonlinearFunctionPtr CGraph::cloneWithVars(VariableConstIterator vbeg, Int *)
  const
{
...
void CGraph::finalize()
{
  ...
  //std::cout << "dq_ has " << dq_.size() << " elements." << std::endl;
  //std::cout << "vq_ has " << vq_.size() << " elements." << std::endl;
}
...
VariablePtr CGraph::getVar(const CNode *cnode)
{
  //ugly and stupid.
  for (VarNodeMap::iterator it=varNode_.begin(); it!=varNode_.end(); ++it) {
    if (it->second==cnode) {
