...
QuadraticFunction::~QuadraticFunction()
{
  ...
  terms_.clear();
  varFreq_.clear();
  for(QfVector::iterator it = subgraphs_.begin(); it != subgraphs_.end();
      ++it) {
    delete *it;
  }
  subgraphs_.clear();
...
QuadraticFunctionPtr
QuadraticFunction::cloneWithVars(VariableConstIterator vbeg) const
{
  QuadraticFunctionPtr qf = (QuadraticFunctionPtr) new QuadraticFunction();
...
void QuadraticFunction::prepJac(VarSetConstIter vbeg, VarSetConstIter vend)
{
  ...
  i = 0;
  for(VariablePairGroupIterator it = terms_.begin(); it != terms_.end(); ++it) {
...
QfVector& QuadraticFunction::getSubgraphs()
{
  return subgraphs_;
}
...
QuadraticFunctionPtr
QuadraticFunction::copyMinus(ConstQuadraticFunctionPtr q2) const
{
...
QuadraticFunctionPtr
QuadraticFunction::copyAdd(ConstQuadraticFunctionPtr q2) const
{
  QuadraticFunctionPtr qf = QuadraticFunctionPtr(); //NULL
  if(!q2) {
    qf = this->clone();
