...
void SimpleTransformer::refNonlinCons_(ConstProblemPtr oldp)
{
  ...
  char cons_type; // 'l' = less than equal, 'g' = greater than equal,
                  // 'e' = equality or range constraint, 'o' = objective
  int error = 0;
  ...
  for(ConstraintConstIterator it = oldp->consBegin(); it != oldp->consEnd();
    ...
    if(c->getConvexity() == Convex) {
      f2 = f->cloneWithVars(newp_->varsBegin(), &error);
      cnew = newp_->newConstraint(f, c->getLb(), c->getUb());
    ...
    if(f && f->getType() != Constant && f->getType() != Linear) {
      ...
      if(qf) {
        // We assume the constraint has at least one lb or ub finite.
        if(c->getLb() > -INFINITY) {
          if(c->getUb() < INFINITY) {
            cons_type = 'e';
          } else {
...
void SimpleTransformer::refQuadCons_(QuadraticFunctionPtr qf,
                                     LinearFunctionPtr& lf, char cons_type)
{
  VariablePtr v = VariablePtr();
  LinearFunctionPtr lfnew;
  QuadraticFunctionPtr qfnew, qf1;
  FunctionPtr fnew;
  ConstraintPtr cnew;
  lf = (LinearFunctionPtr) new LinearFunction();

  for(QfVector::iterator qvit = qf->getSubgraphs().begin();
      qvit != qf->getSubgraphs().end(); ++qvit) {
    qf1 = *qvit;
    if(qf1->isConvex() == Convex && cons_type == 'l') {
      // convex less than equal to constraint
      v = newp_->newVariable();
      ++stats_.nvars;
      ...
      lf->addTerm(v, 1.0);
      continue;
    } else if(qf1->isConvex() == Concave && cons_type == 'g') {
      // concave greater than equal to constraint
      v = newp_->newVariable();
      ++stats_.nvars;
      lfnew = (LinearFunctionPtr) new LinearFunction();
      lfnew->addTerm(v, -1.0);
    ...
    for(VariablePairGroupConstIterator it = qf1->begin(); it != qf1->end();
        ++it) {
                                    ...
                                    int& status)
{
  ...
  cxqHandler_ = (CxQuadHandlerPtr) new CxQuadHandler(env_);
