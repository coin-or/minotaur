...
double SppHeur::addSol_(SolutionPool *spool)
{
  ...
  if (isFeasible_(x)) {
    logger_->msgStream(LogDebug1)
        << me_ << "Found a feasible point with "
...
#if SPEW
    logger_->msgStream(LogDebug1)
        << me_ << "not a set partitioning model" << std::endl;
#endif
    return;
  }
...
void SppHeur::solveLP_(SolutionPool *)
{
}

// void SppHeur::solveLP_(SolutionPool *spool)
// {
//   OsiLPEngine lpe; // TODO: get it from constructor
//   int j = 0;
//   double maxvar = 0.0;
//   const double *x = 0;
//   int fixed = 0;
...
void SppHeur::conWiseNb_(const double *x, NodePtr, RelaxationPtr,
                         SolutionPoolPtr spool)
{
  ConstraintPtr c;
  VariablePtr v;
  ...
  for (ConstraintConstIterator cit = p_->consBegin(); cit != p_->consEnd();
    ...
    logger_->msgStream(LogDebug2)
        << me_ << " constraint " << c->getName() << " has "
        << c->getFunction()->getNumVars() << " variables" << std::endl;
#endif
  }
  ...
  for (ConstraintConstIterator cit = cv.begin(); cit != cv.end(); ++cit) {
...
#if SPEW
    logger_->msgStream(LogDebug2)
        << me_ << " trying constraint number " << j << " out of " << cv.size()
        << " with " << f->getNumVars() << " vars. " << c->getName() << " "
...
#if SPEW
        logger_->msgStream(LogDebug2)
            << me_ << " setting variable " << v->getName()
            << " to 1, i =  " << i << std::endl;
#endif
...
#if SPEW
          logger_->msgStream(LogDebug2)
              << me_ << " propagation found that var "
              << "cannot be 1 " << v->getName()
              << ". Should be caught earlier" << std::endl;
#endif
        } else {
          cfixed = true;
          ++num1;
    ...
    for (vsit = f->varsBegin(); vsit != f->varsEnd(); ++vsit, ++i) {
      ...
      if (10 == val) {
        ...
        logger_->msgStream(LogDebug2)
            << me_ << " variable " << v->getName()
            << " already at 1. Cant happen " << std::endl;
      } else {
#if SPEW
        logger_->msgStream(LogDebug2)
            << me_ << " variable " << v->getName()
            << " already at 0, trying next" << std::endl;
#endif
      }
    }
...
#if SPEW
    logger_->msgStream(LogDebug1)
        << me_ << " Failed to find a feasible solution. " << std::endl;
#endif
  }
}
...
void SppHeur::varWise_(NodePtr, RelaxationPtr, SolutionPoolPtr spool)
{
  ...
  for (VariableConstIterator vit = p_->varsBegin(); vit != p_->varsEnd();
    ...
    if (xx && xx[i] > tol && rand() % 100 < 40 && v->getNumCons() > 3) {
      v->setItmp(0);
      for (ConstrSet::iterator cit2 = (*vit)->consBegin();
           cit2 != (*vit)->consEnd(); ++cit2) {
        (*cit2)->setITemp((*cit2)->getITemp() - 1);
      }
    }
#if SPEW
    logger_->msgStream(LogDebug2)
        << me_ << " variable " << v->getName() << " appears in "
        << v->getNumCons() << " constraints" << std::endl;
#endif
  }
...
#if SPEW
      logger_->msgStream(LogDebug2)
          << me_ << "trying variable " << v->getName() << " appearing in "
          << v->getNumCons() << std::endl;
#endif
      if (true == propVal_(v, num0, consfixed)) {
        ++num1;
#if SPEW
        logger_->msgStream(LogDebug2)
            << me_ << "variable " << v->getName() << " set to 1" << std::endl;
#endif
...
#if SPEW
            logger_->msgStream(LogDebug2)
                << me_ << "constraint " << (*cit)->getName()
                << " has all zeros. stopping. " << std::endl;
#endif
            feas = false;
            break;
          }
  ...
  if (true == feas) {
    addSol_(spool);
    logger_->msgStream(LogDebug1) << me_ << "feasible sol found with " << num1
                                  << " variables" << std::endl;
  } else {
    logger_->msgStream(LogDebug1)
        << me_ << "failed to find a feasible sol" << std::endl;
  }
}
...
void SppHeur::enum_(NodePtr, RelaxationPtr, SolutionPoolPtr spool)
{
  ...
  for (cit = p_->consBegin(); cit != p_->consEnd(); ++cit) {
    c = *cit;
    c->setBFlag(false);
    cv.push_back(c);
  }
  ...
  while (true) {
...
#if SPEW
    logger_->msgStream(LogDebug2)
        << me_ << " trying constraint " << c->getName() << " " << std::endl;
#endif
    ...
    if (cit == cv.end() || false == cfixed || mstack_.size() >= minsize) {
      ...
      if (false == cfixed) {
        // logger_->msgStream(LogInfo) << me_ << (*cit)->getName() << "
        // exhausted " << std::endl;
        if (cit == cv.begin()) {
      ...
      if (mstack_.size() < 100 && cit != cv.end()) {
        logger_->msgStream(LogDebug2)
            << me_ << " depth = " << mstack_.size() << " "
            << (*cit)->getIndex() << " " << env_->getTime() << std::endl;
      }

      for (std::deque<Constraint *>::iterator it = m->cq.begin();
           it != m->cq.end(); ++it) {
        (*it)->setBFlag(false);
      }
      for (std::deque<Variable *>::iterator it = m->vq0.begin();
           it != m->vq0.end(); ++it) {
        (*it)->setItmp(10);
      }
      mstack_.pop_back();
...
void SppHeur::conWise_(NodePtr, RelaxationPtr, SolutionPoolPtr spool)
{
  ...
  if (0 < spool->getNumSols()) {
    ...
    for (VariableConstIterator vit = p_->varsBegin(); vit != p_->varsEnd();
         ++vit, ++j) {
      (*vit)->setItmp(10);  // 10 for FREE, 0 for FIXED at 0, 1 for FIXED at 1
      if (fabs(xx[j] - 1.0) < 1e-6 && (*vit)->getNumCons() > 3 &&
          rand() % 100 < 10) {
        (*vit)->setItmp(0);
        ++num0;
      }
    ...
    logger_->msgStream(LogDebug2)
        << me_ << num0 << " variables at 0. Reference solution value =  "
...
#if SPEW
    logger_->msgStream(LogDebug2)
        << me_ << " trying constraint number " << j << " out of "
        << cv2_.size() << " with " << f->getNumVars() << " vars. "
        << c->getName() << " " << num1 << " vars at 1 and " << num0
        << " vars at 0 " << std::endl;
#endif
...
void SppHeur::conWise2_(NodePtr, RelaxationPtr, SolutionPoolPtr spool)
{
  ...
  while (true) {
...
#if SPEW
      logger_->msgStream(LogDebug1)
          << me_ << " all constraints done " << std::endl;
#endif
      break;
    }
    ...
    logger_->msgStream(LogDebug2)
        << me_ << " trying constraint number " << j << " out of " << cv.size()
        << " with " << f->getNumVars() << " vars. " << c->getName() << " "
        << c->getITemp() << " unfixed vars. " << num1 << " vars at 1 and "
        << num0 << " vars at 0 in model " << std::endl;
#endif
    ...
    i = 0;
    // std::cout << c->getITemp() << " " ;
    for (VariableConstIterator vit = vvec.begin(); vit != vvec.end();
         ++vit, ++i) {
      ...
      if (false == propVal_(v, num0, kk)) {
        ...
        // std::cout << i << " " << vvec.size() << " " << v->getNumCons() <<
        // std::endl;
        cfixed = true;
        ++consfixed;
        ++num1;
...
bool SppHeur::propEnum_(Variable *v, ConstraintConstIterator cit2,
                        VarSetConstIter vit2)
{
  SppHeurMods *m = 0;
  Function *f = 0;
...
bool SppHeur::isSpp_()
{
  ...
  for (ConstraintConstIterator cit = p_->consBegin(); cit != p_->consEnd();
    ...
    if (c->getType() != SetPartition && c->getType() != Aggregation) {
      logger_->msgStream(LogDebug1)
          << me_ << " model is not set partitioning" << std::endl;
      return false;
    }
  }
  logger_->msgStream(LogDebug1)
      << me_ << " model is set partitioning" << std::endl;
  return true;
}
