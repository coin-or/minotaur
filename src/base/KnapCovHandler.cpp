...
KnapCovHandler::KnapCovHandler()
  : env_(EnvPtr()),
    ...
    stats_(0)
{
  // This is an abstract class, find a way to make this work such as
  // Logger(log1).
  //logger_ = (LoggerPtr) new Logger();
  // env_ is a null pointer in this function hence setting a hardcoded value
  // of intTol_. intTol_ =
  // env_->getOptions()->findDouble("int_tol")->getValue();
  intTol_ = 1e-6;
}
...
bool KnapCovHandler::isFeasible(ConstSolutionPtr, RelaxationPtr, bool &,
                                double &)
{
  // no need to check feasibility of knapsack constraints. It is automatically
  // implied by linear handler and integer handler.
  return true;
}
                              ...
                              bool *, SeparationStatus *status)
{
  ...
  if (isintfeas == false) {
    ...
    // Currently CutMan2::addCuts does not add the cuts to the relaxation
    // formulation.
