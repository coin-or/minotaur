...
void NlPresHandler::chkRed_(ProblemPtr p, bool purge_cons, bool* changed, ModQ*,
                            SolveStatus& status)
{
  ConstraintPtr c;
  //  ConBoundModPtr mod;
      ...
      if(c->getUb() == INFINITY && c->getLb() == -INFINITY &&
        ...
        *changed = true;
        ...
        logger_->msgStream(LogDebug2)
            << me_ << " marked constraint " << c->getName() << " for deleting."
            << std::endl;
#endif
      }
    }
    ...
    for(VariableGroupConstIterator it2 = lf->termsBegin();
      ...
      if(nlf) {
        if((z->getType() != Binary && z->getType() != ImplBin) ||
           fabs(z->getUb() - z->getLb() - 1) > eTol_ || p_->isMarkedDel(z) ||
           nlf->hasVar(z)) {
          continue;
        }
      }
...
void NlPresHandler::bin2LinF_(ProblemPtr p, LinearFunctionPtr lf, UInt nz,
                              const UInt* irow, const UInt* jcol,
  ...
  for(UInt i = 0; i < nz; ++i) {
    if(fabs(values[i]) > 1e-12) {
      ...
      if(v1 == v2) {
        ...
        cnew = p->newConstraint(f, -INFINITY, 0.0);
        cnew->setSrcType(ConsHand);
        ...
        lf3->addTerm(v3, -1.0);
        f = (FunctionPtr) new Function(lf3);
        cnew = p->newConstraint(f, -INFINITY, 1.0);
        cnew->setSrcType(ConsHand);
        mod->insert(v3);
      } else {
        ...
        f = (FunctionPtr) new Function(lf3);
        cnew = p->newConstraint(f, -INFINITY, -v1->getLb());
        cnew->setSrcType(ConsHand);
        ...
        f = (FunctionPtr) new Function(lf3);
        cnew = p->newConstraint(f, -INFINITY, v1->getUb());
        cnew->setSrcType(ConsHand);
        ...
        lf3->addTerm(v2, -v1->getUb());
        f = (FunctionPtr) new Function(lf3);
        cnew = p->newConstraint(f, -INFINITY, 0);
        cnew->setSrcType(ConsHand);
...
void NlPresHandler::bin2Lin_(ProblemPtr p, PreModQ* mods, bool* changed)
{
  ...
  if(f) {
    if(Quadratic == f->getType()) {
      hess->fillRowColValues(x, 1.0, mult, values, &err);
      assert(0 == err);
  ...
  for(ConstraintConstIterator cit = p->consBegin(); cit != p->consEnd();
    ...
    if(Quadratic == f->getType()) {
      mult[c->getIndex()] = 1.0;
      memset(values, 0, nz * sizeof(double));
      hess->fillRowColValues(x, 0.0, mult, values, &err);
      assert(0 == err);
      mult[c->getIndex()] = 0.0;
      if(canBin2Lin_(p, nz, irow, jcol, values)) {
        err = 0;
...
#if SPEW
        logger_->msgStream(LogDebug2)
            << me_ << " marked constraint " << c->getName()
...
void NlPresHandler::fixedNl_(ProblemPtr p, bool, bool* changed, ModQ*,
                             SolveStatus& status)
{
  ConstraintPtr c, cnew;
  //  ConBoundModPtr mod;
  ...
  for(ConstraintConstIterator cit = p->consBegin(); cit != p->consEnd();
    ...
    if(0 == error && isfinite(nlfu) && isfinite(nlfl) && nlfu - nlfl < tol) {
      ...
      logger_->msgStream(LogDebug2)
          ...
          << " Removing the nonlinear part." << std::endl;
#endif
      if(lf) {
        lf = lf->clone();
        f = new Function(lf);
        cnew = p->newConstraint(f, c->getLb() - nlfu, c->getUb() - nlfu);
        cnew->setSrcType(ConsTranOrig);
      } else {
        ...
        if(c->getLb() - tol > nlfu || nlfu > c->getUb() + tol) {
          ...
          *changed = true;
          ...
          logger_->msgStream(LogDebug2)
              << me_ << " constraint " << c->getName()
              << " is redundant. Will be removed. " << std::endl;
#endif
        }
      }
      p->markDelete(c);
...
void NlPresHandler::perspRef_(ProblemPtr p, PreModQ*, bool* changed)
{
  ...
  for(ConstraintConstIterator cit = p->consBegin(); cit != p->consEnd();
    ...
    for(VarSetIter it = nlvars.begin(); it != nlvars.end(); ++it) {
      ...
      candvars = indvars;
      indvars.clear();
      for(ConstrSet::iterator cit2 = v->consBegin();
          cit2 != v->consEnd() && candvars.size() > 0; ++cit2) {
        c2 = *cit2;
        if(c2->getFunctionType() != Linear) {
          continue;
        }
...
#if SPEW
      logger_->msgStream(LogDebug2)
          << me_ << "found a candidate for "
          << "nonlinear perspective reformulation" << std::endl;
      c->write(logger_->msgStream(LogDebug2));
      logger_->msgStream(LogDebug2)
          << me_ << "Indicator Variables:" << std::endl;
...
SolveStatus NlPresHandler::presolve(PreModQ* mods, bool* changed0, Solution**)
{
  ...
  while(changed == true && stats_.iters < 5) {
    logger_->msgStream(LogDebug2)
        << me_ << "starting presolve iter " << stats_.iters << std::endl;
    changed = false;
  ...
  if(f && ub < INFINITY) {
    ...
    if(lf) {
      ...
      for(VariableGroupConstIterator it2 = lf->termsBegin();
        ...
        z = it2->first;
        if((z->getType() == Binary || z->getType() == ImplBin) &&
           (z->getUb() - z->getLb()) > eTol_ && !(p->isMarkedDel(z))) {
          a0 = it2->second;
          if(a0 > 0 && olb + a0 > ub) {
            *changed = true;
                               ...
                               double* u)
{
  ...
  if(a == 0) {
    ...
    *u = std::max(v->getLb() * (a * v->getLb() + b),
                  v->getUb() * (a * v->getUb() + b));
    *l = std::min(v->getLb() * (a * v->getLb() + b),
                  v->getUb() * (a * v->getUb() + b));
...
void NlPresHandler::quad_bound_range(VariablePtr v, double a, double blb,
                                     double bub, double* l, double* u)
{
  double llb;
  double lub;
  double ulb;
...
void NlPresHandler::linear_var_set(VariableSet lfvars, FunctionPtr f,
...
void NlPresHandler::get_other_linear_var_bounds(VariablePtr v, double* lflb,
                                                double* lfub,
                                                VariableSet linear_terms,
                                                LinearFunctionPtr lf)
{
  ...
  double lb = 0;
  double ub = 0;
  for(VariableSet::iterator it = linear_terms.begin(); it != linear_terms.end();
      ++it) {
    v2 = *it;
    if(v2 != v) {
      a = lf->getWeight(v2);
...
void NlPresHandler::get_other_quad_var_bounds(VariablePtr v, double* qflb,
                                              double* qfub, VariableSet qfvars,
                                              QuadraticFunctionPtr qf,
                                              LinearFunctionPtr lf)
{
  VariablePtr v2;
...
void NlPresHandler::bilinear_bounds(VariablePtr v, double* qflb1, double* qfub1,
                                    VariableSet qfvars, QuadraticFunctionPtr qf)
{
...
void NlPresHandler::lin_var_bound(VarBoundModVector lfmod, LinearFunctionPtr lf,
                                  QuadraticFunctionPtr qf, VariableSet qfvars,
                                  VariableSet linear_terms, double cl,
                                  double cu, int& change)
{
  ...
  if(false == lfmod.empty()) {
    for(VarBoundModVector::iterator it = lfmod.begin(); it != lfmod.end();
        ++it) {
                                   ...
                                   double cu, int& change)
{
  ...
  for(VariableSet::iterator it = linear_terms.begin(); it != linear_terms.end();
    ...
    v = *it;
    lflb +=
        std::min(lf->getWeight(v) * v->getLb(), lf->getWeight(v) * v->getUb());
    lfub +=
        std::max(lf->getWeight(v) * v->getLb(), lf->getWeight(v) * v->getUb());
  }
  ...
  if(false == qfmod.empty()) {
    for(VarBoundModVector::iterator it = qfmod.begin(); it != qfmod.end();
        ++it) {
      (*it)->applyToProblem(p_);
      ++stats_.vBnd;
    }
  ...
  for(ConstraintConstIterator cit = p->consBegin(); cit != p->consEnd();
    ...
    if(c->getFunctionType() != Linear && c->getFunctionType() != Constant) {
      ...
      if(false == dmods.empty()) {
        for(VarBoundModVector::iterator it = dmods.begin(); it != dmods.end();
            ++it) {
          (*it)->applyToProblem(p);
          ++stats_.vBnd;
#if SPEW
...
void NlPresHandler::writePreStats(std::ostream& out) const
{
  out << me_ << "Statistics for presolve by NlPresHandler:" << std::endl
      ...
      << me_ << "Number of variables deleted    = " << stats_.varDel
      << std::endl
      << me_ << "Number of constraints deleted  = " << stats_.conDel
      << std::endl
      << me_ << "Number of constraints relaxed  = " << stats_.conRel
      << std::endl
      ...
      << me_ << "Changes in nodes               = " << stats_.nMods
      << std::endl;
}
