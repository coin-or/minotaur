...
void QGHandler::addInitLinearX_(const double* x)
{
  ...
  if(oNl_) {
    ...
    if(error == 0) {
      ...
      logger_->msgStream(LogError)
          << me_ << "Objective not defined at this point." << std::endl;
    }
  }
  return;
}
                           ...
                           SeparationStatus* status)
{
  ...
  default:
    logger_->msgStream(LogError)
        ...
        << std::endl;
    logger_->msgStream(LogError)
        << me_ << "No cut generated, may cycle!" << std::endl;
    *status = SepaError;
    break;
  }

  if(*status == SepaContinue || *status == SepaError) {
...
bool QGHandler::isFeasible(ConstSolutionPtr sol, RelaxationPtr, bool&, double&)
{
  int error = 0;
  double act, cUb;
  ConstraintPtr c;
...
void QGHandler::linearizeObj_()
{
  ...
  if(!o) {
    ...
    VariablePtr vPtr =
        rel_->newVariable(-INFINITY, INFINITY, Continuous, name, VarHand);
    objVar_ = vPtr;
                          ...
                          double* c, LinearFunctionPtr* lf, int* error)
{
  ...
  if(*error == 0) {
    ...
    *c = fval - InnerProduct(x, a, minlp_->getNumVars());
    ...
    logger_->msgStream(LogError)
        << me_ << "gradient not defined at this point." << std::endl;
  }
  delete[] a;
  return;
}
                             ...
                             SeparationStatus* status)
{
  ...
  if(oNl_) {
    ...
    if(error == 0) {
      ...
      logger_->msgStream(LogError)
          << me_ << " objective not defined at this solution point."
          << std::endl;
    }
  }
  return;
...
void QGHandler::cutToObj_(const double* nlpx, const double* lpx, CutManager*,
                          SeparationStatus* status)
{
  if(oNl_) {
    int error = 0;
      ...
      if((vio > solAbsTol_) &&
        ...
        if(error == 0) {
          ...
          if(error == 0) {
            ...
            if((vio > solAbsTol_) &&
               ((relobj_ - c) == 0 || vio > fabs(relobj_ - c) * solRelTol_)) {
              ++(stats_->cuts);
              sstm << "_qgObjCut_" << stats_->cuts;
              lf->addTerm(objVar_, -1.0);
              *status = SepaResolve;
                         ...
                         ModVector&, bool* sol_found, SeparationStatus* status)
{
  ...
  *status = SepaContinue;
  for(VariableConstIterator v_iter = rel->varsBegin(); v_iter != rel->varsEnd();
      ++v_iter) {
    var = *v_iter;
    v_type = var->getType();
    if(v_type == Binary || v_type == Integer) {
...
void QGHandler::writeStats(std::ostream& out) const
{
  out << me_ << "number of nlps solved                       = " << stats_->nlpS
      << std::endl
      << me_ << "number of infeasible nlps                   = " << stats_->nlpI
      << std::endl
      << me_ << "number of feasible nlps                     = " << stats_->nlpF
      << std::endl
      << me_
      << "number of nlps hit engine iterations limit  = " << stats_->nlpIL
      << std::endl
      << me_ << "number of cuts added                        = " << stats_->cuts
      << std::endl;
  return;
}
