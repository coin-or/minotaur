...
/**
 * \file TreeManager.h
 * \brief Declare class ParTreeManager for managing tree for parallel Branch-and-Bound.
...
namespace Minotaur {
  ...
  public:
    ...
    /**
     * \brief Return the gap between the lower and upper bound as a
     * percentage, given a lower bound, when executing in parallel mode. It is
     * calculated as
     * \f$\frac{ub-lb}{\left|ub\right|+\epsilon}\times 100\f$.
     * \param[in] treeLb is the lower bound of the tree, considering active
     * nodes and other open nodes that have not been sent to the node pool.
     */
