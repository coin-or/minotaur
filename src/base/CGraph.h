...
namespace Minotaur {
  ...
  public:
    ...
    NonlinearFunctionPtr getPersp(VariablePtr z, double eps, int *err,
    ...
    void evalHessian(double mult, const double *x, const LTHessStor *stor,
    ...
    /**
     * After adding all the nodes of the graph, finalize is called to create
     * the forward and backward traversal queues, and related book-keeping.
     */
    void finalize();
     ...
     * never be NULL.
     * \param [in] rchild If the opcode needs two children, e.g. OpPlus,
     * OpMult etc, then rchild should be the right child. For univariate
     * functions like OpSqrt, it should be NULL.
     ...
     *
     * \param [in] op OpCode of the new node
     * \param [in] child An array of pointers to children. It should be size
     * 'n'.
    ...
    /**
     * \brief Create a new node with constant integer value. It does not have
     * any children.
     *
    ...
    /**
     * \brief Create a new node denoting an input variable. It does not have
     * any children.
     *
  ...
  private:
    ...
    /// All dependent nodes, i.e. nodes with OpCode different from OpVar,
    /// OpInt and OpNum.
    CNodeQ dq_;
    ...
    /// Recursive function to check whether CGraph represents a sum of
    /// squares.
     ...
     *
     *  \param [out] error Set to a nonzero if an error occurs. Otherwise,
     * leave it unchanged.
     */
    void grad_(int *error);

    void setupHess_(VariablePtr v, CNode *node,
                    std::set<ConstVariablePair, CompareVariablePair> &vps);
