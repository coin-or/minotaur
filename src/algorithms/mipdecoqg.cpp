...
int main(int argc, char* argv[])
{
  ...
  iface = (MINOTAUR_AMPL::AMPLInterfacePtr) 
    new MINOTAUR_AMPL::AMPLInterface(env, "PDEqg");
  ...
  if (Finished != pres->getStatus() && NotStarted != pres->getStatus()) {
   ...
   if (options->findBool("solve")->getValue()==true) {
    ...
    PDEqg_hand = (QGHandlerPDEPtr) new QGHandlerPDE(env, inst, lin_e); 
    PDEqg_hand->setModFlags(false, true);
    handlers.push_back(PDEqg_hand);
