...
IpoptSolution::IpoptSolution(const double* x, double objval, ProblemPtr problem)
...
void IpoptEngine::changeConstraint(ConstraintPtr, LinearFunctionPtr, double,
                                   double)
{
  consChanged_ = true;
}
  ...
  if(ws) {
    ...
    if(!useWs_) {
      logger_->msgStream(LogInfo)
          << me_ << "setWarmStart() method is called "
...
EngineStatus IpoptEngine::solve()
{
  ...
  logger_->msgStream(LogDebug)
      << me_ << "time taken = " << timer_->query() << std::endl;
  logger_->msgStream(LogDebug)
      << me_ << "Ipopt's status = " << status << std::endl;
#endif
  ...
  case Ipopt::Feasible_Point_Found:
    logger_->msgStream(LogInfo) << me_ << "Feasible point found" << std::endl;
    status_ = ProvenLocalOptimal;
    sol_ = mynlp_->getSolution();
    break;
...
#if SPEW
  logger_->msgStream(LogDebug)
      << me_ << "solve number = " << stats_->calls << std::endl;
  logger_->msgStream(LogDebug)
      << me_ << "number of iterations = " << iters << std::endl;
  logger_->msgStream(LogDebug)
      << me_ << "status = " << getStatusString() << std::endl;
...
#ifdef NDEBUG
bool IpoptFunInterface::get_bounds_info(Index, Number* x_l, Number* x_u, Index,
...
#endif
{
  ...
  if(init_z == false || init_lambda == false) {
    double* xp = x;
#if SPEW
    logger_->msgStream(Minotaur::LogDebug2)
        << "IpoptFunInterface: "
        << "setting initial point using the values "
        << " available in the Problem object." << std::endl;
#endif
    ...
    logger_->msgStream(Minotaur::LogDebug2)
        << "IpoptFunInterface: "
        << "setting initial point using the values "
        << " available from previous solve." << std::endl;
#endif
...
bool IpoptFunInterface::eval_f(Index n, const Number* x, bool,
                               Number& obj_value)
{
  int error = 0;
...
bool IpoptFunInterface::eval_h(Index n, const Number* x, bool,
                               Number obj_factor, Index, const Number* lambda,
                               bool, Index, Index* iRow, Index* jCol,
                               Number* values)
{
  int error = 0;
  double* ex = NULL; // it will be assigned to xx if evalWithinBnds_ is true,
  ...
  if(x == 0 && lambda == 0 && values == 0) {
    ...
    problem_->getHessian()->fillRowColValues(
        ex, (double)obj_factor, (double*)lambda, (double*)values, &error);
...
bool IpoptFunInterface::eval_jac_g(Index n, const Number* x, bool, Index, Index,
                                   Index* iRow, Index* jCol, Number* values)
{
  ...
  if(values == 0) {
    ...
    problem_->getJacobian()->fillRowColValues((double*)ex, (double*)values,
                                              &error);
    if(xx) {
      delete[] xx;
    }
  ...
  if(error != 0) {
    logger_->msgStream(Minotaur::LogError)
        << "IpoptFunInterface: "
